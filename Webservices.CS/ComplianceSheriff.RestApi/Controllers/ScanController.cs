...
namespace ComplianceSheriff.Controllers
{
    ...
    public class ScanController : Controller
    {
        ...
        [HttpPost("recentScans")]
        [ProducesResponseType(200, Type = typeof(RecentScanResponse))]
        public async Task<IActionResult> GetRecentScansList([FromBody] RecentScanRequest request,
                                                       ...
                                                       CancellationToken cancellationToken)
        {
            ...
            var recentScanRequestModel = new RecentScanRequestModel
            {
                CurrentPage = request.CurrentPage,
                RecordsToReturn = request.RecordsToReturn,
                UserGroupId = userGroupId,
                SortColumn = request.SortColumn,
                SortDirection = request.SortDirection
            };

            var result = await accessor.GetRecentScans(recentScanRequestModel, cancellationToken);
            return Ok(result);
        }
        ...
        public async Task<IActionResult> GetScanById(string scanId,
                                                        ...
                                                        [FromServices] ITextFormatterService textFormatterService,
                                                        [FromServices] IModelStateService modelStateService)
        {
            ...
            try
            {
                ...
                result.IncludedDomains = textFormatterService.IncludedDomainsFormatForResponse(result.IncludedDomains);
                if (result == null)
                                                        ...
                                                        [FromServices] IModelStateService modelStateService)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    scanResult.IncludedDomains = textFormatterService.IncludedDomainsFormatForResponse(scanResult.IncludedDomains);
                                                    ...
                                                    CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    return NotFound(new ApiResponse { StatusCode = 404, ErrorCode = "api:scan:createScan:checkpointGroupListNotAvailable" });
                }
                ...
                foreach (string chkGrpId in request.CheckpointGroupIds)
                {
                    if (!chkGroupIdList.Contains(chkGrpId))
                    {
                        ...
                        return NotFound(new ApiResponse { StatusCode = 404, ErrorCode = "api:scan:createScan:checkpointGroupIdNotAvailable" });
                    }
                }
                ...
                request.IncludedDomains = !String.IsNullOrWhiteSpace(request.IncludedDomains) ? textFormatterService.IncludedDomainsFormatForDb(request.IncludedDomains) : "";
                ...
                if (scanId > 0)
                {
                    ...
                    return BadRequest(new ApiResponse { StatusCode = 400, ErrorCode = "api:scan:createScan:scanIsNotCreated" });
            }
            catch (Exception ex)
            {
                _logger.LogError($"{ex.Message} {ex.StackTrace}");
            ...
            return StatusCode(201, new ApiResponse { StatusCode = 201, Data = new Dictionary<string, object> {{ "scanId", scanId }} });
        }
                                                           ...
                                                           CancellationToken cancellationToken)
        {
            ...
            return Ok(new ApiResponse { StatusCode = 200, Data = new Dictionary<string, object> { { "completed", "success" } } });
        }
                                                    ...
                                                    CancellationToken cancellationToken)
        {
            ...
            if (!scanIdIsValid)
            {
                return BadRequest(new ApiResponse { StatusCode = 400, ErrorCode = "api:scan:runScan:invalidScanId" });
            }
            ...
            if (!scanExists)
            {
                return NotFound(new ApiResponse { StatusCode = 404, ErrorCode = "api:scan:runScan:scanNotFound" });
            }
            ...
            if (status != 0 && status != 2 && status != 3 && status != 8)
            {
                return BadRequest(new ApiResponse { StatusCode = 400, ErrorCode = "api:scan:runScan:unableToRunScanDueToCurrentStatus" });
            }
            ...
            return StatusCode(201, new ApiResponse { StatusCode = 201, Data = new Dictionary<string, object> { { "runId", runId } } });
        }
                                                       ...
                                                       CancellationToken cancellationToken)
        {
            ...
            if (!scanIdIsValid)
            {
                return BadRequest(new ApiResponse { StatusCode = 400, ErrorCode = "api:scan:runStatus:invalidScanId" });
            }
            ...
            if (!scanExists)
            {
                return NotFound(new ApiResponse { StatusCode = 404, ErrorCode = "api:scan:runStatus:scanNotFound" });
            }
            ...
            return Ok(new ApiResponse { StatusCode = 200, Data = new Dictionary<string, object> { { "runId", runStatus } } });
        }
