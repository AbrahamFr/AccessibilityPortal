    ...
    public class AuditReportController : Controller
    {
            ...
            try
            {
                ...
                if (!(request.AuditTypeId > 0 && request.AuditTypeId < 5))
                {
                    ...
                    var additionalData = new Dictionary<string, object>
                        {
                            { "ReportTypeId", request.AuditTypeId }
                        };
                    return StatusCode(400, new ApiResponse { StatusCode = 400, ErrorCode = responseCode, Data = additionalData });
                }
                                                       ...
                                                       CancellationToken cancellationToken)
        {
            ...
            if (!System.IO.File.Exists(result.FileLocation))
            {
                ...
                var additionalData = new Dictionary<string, object>
                {
                    { "fileName", Path.GetFileName(result.FileLocation) }
                };
                _logger.LogError($"{responseCode} : {result.FileLocation} not found on the server");
                return StatusCode(404, new ApiResponse { StatusCode = 404, ErrorCode = responseCode, Data = additionalData });
            }
