...
{
    internal class SchedulingMainDb
    {
        ...
        internal SchedulingMainDb(IClusterDataOptions dataOptions, ILogger logger, SqlConnection conn)
        {
        ...
        private void CreateDatabaseIfMissing()
        {
            ...
            if (DatabaseConnection.CheckIfDatabaseNeedsToBeCreated(conn, "main", dataOptions, logger))
            {
        ...
        {
            // If they exist, manually create EF Migrations table with Initial entry
            if (IsTablePresent($"{databaseName}.dbo.ScheduledTasks") && !IsTablePresent($"{databaseName}.dbo.__EFMigrationsHistory"))
            {
                logger.LogInformation($"Upgrade detected without EF. Database table '{databaseName}.dbo.__EFMigrationsHistory' needs to be created.");
